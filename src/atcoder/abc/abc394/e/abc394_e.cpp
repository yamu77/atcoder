...
int dp(int n, vector<vector<char>> graph, int start, int goal)
{
    vector<map<int, string>> s;
    int current = start;
    // 最初の経路
    rep(i, n)
    {
        if (graph.at(current).at(i) != '-')
        {
            map<int, string> s_tmp;
            s_tmp[i] = graph.at(current).at(i);
            s.push_back(s_tmp);
        }
    }
    while (true)
    {
        vector<map<int, string>> s_next;
        for (auto s_tmp : s)
        {
            int current_tmp = s_tmp.begin()->first;
            string strings = s_tmp.begin()->second;
            rep(i, n)
            {
                if (graph.at(current_tmp).at(i) != '-')
                {
                    strings += graph.at(current_tmp).at(i);
                    s_tmp[i] = strings;
                    s_next.push_back(s_tmp);
                }
                if (i == goal)
                {
                    return strings.size();
